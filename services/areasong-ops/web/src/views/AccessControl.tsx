...
interface AccessControlProps {
  ...
  busy: string
  onRefresh: () => void
  onSave: (body: AccessControlUpdate) => Promise<void>
...
export function AccessControl({
  access, loading, available, error, busy, onRefresh, onSave,
  ...
  async function savePolicy() {
    if (!access) return
    await onSave({ enforced: effectiveEnforced, expectedVersion: access.version })
    setEnforced(null)
  }
  ...
  return (
    <div className="page">
      ...
      {available && access && <>
        <section className="access-summary"><div className="access-summary-main"><span className="access-icon"><ShieldCheck size={20} /></span><div><h2>{effectiveEnforced ? '访问策略已强制执行' : '访问策略兼容模式'}</h2><p>默认租户：{access.defaultTenant || '未设置'} · 版本 {access.version ?? 0} · {shortHash(access.digest)}</p></div><span className={effectiveEnforced ? 'credential-health healthy' : 'credential-health warning'}>{effectiveEnforced ? 'Enforced' : '兼容'}</span></div><div className="access-summary-actions"><button className="button secondary" type="button" onClick={toggleEnforced}>{effectiveEnforced ? '切换兼容模式' : '启用强制执行'}</button><button className="button secondary" type="button" onClick={() => void savePolicy()} disabled={busy === 'access' || enforced === null}><Save size={14} />保存策略</button></div></section>
