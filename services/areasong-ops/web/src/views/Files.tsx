      ...
      {available && <>
        <section className="page-section no-top-gap">
          ...
          <div className="config-actions">
            ...
            <button className="button secondary" type="button" disabled={!rootId || busy === 'files-read'} onClick={() => void read()}><FolderOpen size={14} />{busy === 'files-read' ? '读取中' : '读取'}</button>
          </div>
        </section>
        ...
        {file && <section className="page-section">
          ...
          {file.isDirectory
            ? <div className="operation-list">{(file.entries ?? []).length === 0 ? <div className="empty-state compact">目录为空</div> : file.entries?.map((entry) => <button className="operation-row" type="button" key={entry.path} onClick={() => { setPath(entry.path); void onRead(file.rootId, entry.path) }}><span><strong>{entry.name}</strong><small>{entry.isDirectory ? '目录' : '文件'} · {formatTime(entry.modifiedAt)}</small></span><code>{entry.path}</code><span>{entry.size} bytes</span><span>{entry.isDirectory ? '打开' : '读取'}</span></button>)}</div>
            : <><label className="config-editor"><span>当前内容</span><textarea value={content} onChange={(event) => setContent(event.target.value)} readOnly={file.readOnly} spellCheck={false} rows={16} /></label><div className="config-actions"><button className="button danger" type="button" disabled={file.readOnly || !file.digest || !content || busy === 'files-propose'} onClick={() => void propose()}><Save size={14} />{busy === 'files-propose' ? '提案中' : '生成文件提案'}</button></div></>}
        </section>}
