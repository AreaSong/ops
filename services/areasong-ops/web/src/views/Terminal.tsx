...
export function Terminal({
  ...
  const [planConfirmations, setPlanConfirmations] = useState<
  ...
  async function createPlan(event: FormEvent) {
    ...
    if (!objectId || !shellInput || requestConfirmation !== requestPhrase)
      return;
    try {
      await onCreatePlan({
        objectId,
  ...
  return (
    <div className="page">
      <header className="page-header">
        <div>
          <span className="eyebrow">Break-glass 受控执行</span>
          <h1>紧急终端</h1>
        </div>
        <button
      ...
      {available && loading && plans.length === 0 && (
        ...
        <>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setShellInput(
                        (current) =>
                          current ||
                          `${item.name}${item.arguments?.length ? ` ${item.arguments.join(" ")}` : ""}`,
                      )
                    }
                    title="将命令模板放入 Shell 计划"
                  >
                    <span>
                      <strong>{item.name}</strong>
                      <small>
                        {item.executable} ·{" "}
                        {item.readOnly ? "只读" : "受控写操作"}
                      </small>
                    </span>
                    <code>{item.arguments?.join(" ") || "无参数"}</code>
                    <span>{item.timeoutSeconds}s</span>
                    <span>{item.readOnly ? "只读" : "高风险"}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          {breakGlassAvailable && (
            <section className="page-section">
              <div className="section-heading">
                <h2>创建 Shell 计划</h2>
                <span>独立批准人确认后由创建人执行</span>
              </div>
              <form
                className="runner-update-form"
                onSubmit={(event) => runAction(createPlan(event))}
              >
                <label>
                  <span>对象 ID</span>
                  <input
                ...
                <label className="wide">
                  ...
                  <textarea
                    ...
                    spellCheck={false}
                    placeholder="仅填写必要的受控命令"
                  />
                </label>
                <label className="wide runner-prepare-confirm">
                  <span>申请确认</span>
                  <code>{requestPhrase || "先填写对象 ID"}</code>
                  <input
                    required
                    value={requestConfirmation}
                    onChange={(event) =>
                      setRequestConfirmation(event.target.value)
                    }
                  />
                </label>
                <div className="form-actions">
                  <button
                    ...
                    disabled={
                      !objectId ||
                      !shellInput ||
                      requestConfirmation !== requestPhrase ||
                      busy === "terminal-create"
                    }
                  >
                    <ShieldCheck size={14} />
                    {busy === "terminal-create" ? "提交中" : "创建待批准计划"}
                  </button>
                </div>
              </form>
            </section>
          ...
          {!breakGlassAvailable && (
            <div className="recovery-warning">
              <ShieldCheck size={15} />
              Break-glass Shell 当前关闭；只读命令仍可按权限执行。
            </div>
          )}
          ...
          {onRun && (
            <section className="page-section">
              <div className="section-heading">
                <h2>受控命令会话</h2>
                <span>只使用命令目录中的固定能力</span>
              </div>
              <form
                className="runner-update-form"
                onSubmit={(event) => runAction(runCommand(event))}
              >
                <label>
                  <span>对象 ID</span>
                  <input
                ...
                <label className="wide">
                  <span>直接会话确认（若后端要求）</span>
                  <input
                    value={commandConfirmation}
                    onChange={(event) =>
                      setCommandConfirmation(event.target.value)
                    }
                  />
                </label>
                <div className="form-actions">
                  <button
                    ...
                    type="submit"
                    disabled={!objectId || !command || busy === "terminal-run"}
                  >
                    <Play size={14} />
                    {busy === "terminal-run" ? "执行中" : "运行受控命令"}
                  </button>
                </div>
              </form>
              {lastOutput && (
                <pre className="terminal-output">
                  <code>{lastOutput.output || "(无输出)"}</code>
                </pre>
              )}
            </section>
          )}
          ...
          {breakGlassAvailable && (
            <section className="page-section">
              ...
              {plans.length === 0 ? (
                <div className="empty-state compact">暂无终端计划</div>
              ) : (
                <div className="runner-update-list">
                  {plans.map((plan) => {
                    ...
                    const twoParty = plan.approvalPolicy === "two_party_v1";
                    const approvalPending =
                      plan.state === "pending_approval" ||
                      (!twoParty && plan.state === "pending_second_approval");
                    const approval =
                      approvalPending &&
                      canCurrentActorApprove(plan, currentActorHash);
                    const approvalLabel = twoParty
                      ? "独立批准"
                      ...
                      >
                        <header>
                          <div className="runner-update-title">
                            <span
                              className={`service-indicator ${stateTone(plan.state)}`}
                            />
                            <div>
                              <strong>{plan.objectId}</strong>
                              <small>
                                {stateLabel[plan.state] || plan.state} ·{" "}
                                {formatTime(plan.createdAt)}
                              </small>
                            </div>
                          </div>
                          <span
                            className={`credential-health ${stateTone(plan.state)}`}
                          >
                            {stateLabel[plan.state] || plan.state}
                          </span>
                        </header>
                        <dl className="runner-update-detail">
                          <div>
                            <dt>输入摘要</dt>
                            <dd>
                              <code>{shortHash(plan.inputDigest)}</code>
                            </dd>
                          </div>
                          <div>
                            <dt>计划 ID</dt>
                          ...
                          <div>
                            <dt>创建人</dt>
                            <dd>
                              <code>{shortHash(plan.actorHash)}</code>
                            </dd>
                          </div>
                          <div>
                            <dt>独立批准人</dt>
                          ...
                          {!twoParty && (
                            <div>
                              ...
                              <dd>
                                <code>
                                  {shortHash(plan.secondApprovedByHash)}
                                </code>
                              </dd>
                            </div>
                          )}
                          <div>
                            <dt>结束时间</dt>
                            <dd>{formatTime(plan.finishedAt)}</dd>
                          </div>
                        </dl>
                        {approval && (
                          <div className="runner-update-actions attention">
                            <label>
                              <span>{approvalLabel}确认</span>
                              <code>{phrase}</code>
                              <input
                                value={confirmation}
                                onChange={(event) =>
                            ...
                            <button
                              ...
                              disabled={
                                !phrase ||
                                confirmation !== phrase ||
                                busy === `terminal-approve:${plan.id}`
                              }
                              onClick={() => runAction(approve(plan))}
                            >
                              <ShieldCheck size={14} />
                              {busy === `terminal-approve:${plan.id}`
                                ? "批准中"
                        ...
                        {executable && (
                          <div className="runner-update-actions attention">
                            <label>
                              <span>执行时再次提供原始输入</span>
                              <textarea
                                rows={3}
                                value={input}
                                onChange={(event) =>
                                  setExecutionInput(plan.id, event.target.value)
                                }
                                spellCheck={false}
                              />
                            </label>
                            <button
                              ...
                              disabled={
                                !input ||
                                busy === `terminal-execute:${plan.id}` ||
                                !canCurrentActorExecute(
                                  plan,
                                  currentActorHash,
                                )
                              }
                              onClick={() => runAction(execute(plan))}
                            >
                              <Play size={14} />
                              {busy === `terminal-execute:${plan.id}`
                                ? "执行中"
