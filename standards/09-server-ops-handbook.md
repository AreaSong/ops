...
### 1.4 与 AI 运维助手配合使用（指令与沟通）

本文的执行方可以是人，也可以是 AI 运维助手（Agent）。配合助手使用时的**交互模型**：

> 你下达场景指令 → 助手读取本文对应章节执行 → 只在三类点位暂停等你：
> **选型决策**（文档中的选项对比表）、**信息缺口**（哪台机器/哪个环境/业务约束）、**变更批准**（一切写操作）。

P0 基线项不是商量对象——助手不应问"要不要做"，只应问"什么时候做"（排期）。

**场景指令速查**（可直接复制，替换机器名）：

| 场景 | 指令示例 | 交互流程 |
|------|---------|---------|
| 摸底存量机器 | 按 standards/09 附录 A 对 prod-web-01 摸底，只读检查，输出达标/不达标/不适用三态清单和 P0/P1/P2 整改建议 | 全程只读、不提问；拿到清单后你说"按 P0 开始整改"，进入逐项批准执行 |
| 新机上线 | 新机器 prod-api-03 按 standards/09 附录 C 的 Day-0 清单初始化 | 助手先集中问完选型题（编排路线/防火墙/访问路径等），你答复后照清单执行，每个变更点等批准 |
| 技术选型 | 按 standards/09 第 16 章帮我选编排方案 | 助手按矩阵问规模/团队/服务形态，给推荐结论与理由，你确认后进入部署规划 |
| 例行运维 | 执行 standards/09 附录 D 的月度事项 | 逐项执行只读检查并汇报，异常项转整改建议 |
| 故障处理 | prod-db-01 磁盘满了，按 standards/09 第 29 章处理 | 助手先给止血方案等批准（故障期沟通从简），恢复后 48h 内出复盘 |
| 单项工作 | 按 standards/09 第 13 章检查这台机器的凭证管理 | 按该章清单核对并汇报 |

**沟通规则——对下指令的人**：

1. 显式引用依据（"按 standards/09 第 X 章 / 附录 X"），不让助手猜标准
2. 一次一个场景、一台机器起步；跨多台的操作按第 30 章批量纪律灰度推进
3. 答复选型题：直接选选项；要偏离本文推荐结论时说明理由，偏离决策记入台账/变更单
4. 批准变更前看齐五要素（第 20 章）：做什么/为什么/影响范围/回滚/验证，缺一项就要求补全再批

**沟通规则——对助手（执行方约束）**：

1. 变更类操作先给五要素再等批准，批准一次只覆盖一次操作，不得视为长期授权
2. 只在三类点位提问；P0 项不询问"是否要做"
3. 执行结果对照对应章节清单逐项回报，不达标项不得静默跳过
4. 完成后提醒更新台账（inventory/）并 Git 提交
5. 新会话开始时先读台账与本文再动手，不凭记忆操作
...
- 2026-07-05 补充 1.4 节：与 AI 运维助手配合使用的指令速查与双向沟通规则
