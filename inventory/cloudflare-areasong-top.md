...
本台账基于服务器侧 Nginx 配置、源站证书、公开 DNS、公开 HTTPS 响应头、源站 SNI 握手结果和 Cloudflare 控制面核对结果整理。2026-07-29 已按批准为 Grafana 创建 Access Application、人员与自动化策略及 service token；2026-08-10 已为 AreaSong Ops 创建独立 Access Application 和仅允许指定邮箱的 OTP 策略。token secret 不进入本台账或 Git。
...
| `ops.areasong.top` | AreaSong Ops 控制面 | Cloudflare 代理并受独立 Access Application 保护，A/AAAA 返回 Cloudflare 边缘地址 | 未认证请求进入 Access 登录；`song80184@gmail.com` 完成 OTP 后进入控制面 | `127.0.0.1:3200` |
...
- `ops.areasong.top` 已完成公开 DNS、Access 登录、已认证控制面、源站直连拒绝和 Blackbox Access 策略探测的交叉验收。
...
| `ops.areasong.top` | `/etc/ssl/cf/top/origin.pem` | Cloudflare Origin CA，SAN `*.areasong.top`、`areasong.top` | 2026-07-01 至 2041-06-27 | Cloudflare 代理域名使用 Origin Certificate；Nginx 仅允许 Cloudflare 官方 CIDR 回源 |
...
| `ops.areasong.top` | Let's Encrypt `YE2` | `*.areasong.top`、`areasong.top` | Cloudflare 边缘证书；公开 TLS 链路已验收 |
...
| 项目 | 当前状态 |
| --- | --- |
| Cloudflare Origin Certificate | 控制台创建人和轮换负责人均为 `as`；当前用于 `resume.areasong.top`、`sorryiossearch.areasong.top`、`monitor.areasong.top`、`forge.areasong.top`、`ops.areasong.top`；长期有效至 2041-06-27；过期前 180/90/30/7 天提醒 |
...
| 对象 | 用途 | 创建人 | 负责人 | 提醒 / 处置 |
| --- | --- | --- | --- | --- |
| Cloudflare Origin Certificate `*.areasong.top` / `areasong.top` | `resume.areasong.top`、`sorryiossearch.areasong.top`、`monitor.areasong.top`、`forge.areasong.top`、`ops.areasong.top` 的代理回源 TLS 证书 | `as` | `as` | 2041-06-27 到期；Prometheus 已落地 180/90/30/7 天分级提醒，30 天内安排轮换，7 天内按紧急处理 |
...
| Access Application `AreaSong Ops - ops.areasong.top` | AreaSong Ops 人员身份边界 | `as` | `as` | 仅允许 `song80184@gmail.com` OTP、会话 6 小时；不复用 Grafana service token、不创建 Bypass |
...
| `ops.areasong.top` Access Application | self-hosted；仅允许 `song80184@gmail.com` OTP；6 小时会话；无 Bypass/service token | **已生效**：独立 Application 与 Allow 策略 | 未认证公网请求 302；允许邮箱完成 OTP 后 `/api/session` 与页面身份一致；源站直连 403；JWT、Origin 与 CSRF 边界由生产抽测和自动化测试共同验收 |
...
2026-07-04 通过 Cloudflare 控制台只读核对 Zone 配置；2026-07-18 追加核对时 Access Application 与 service token 均为空；2026-07-29 按批准完成 Grafana Access 创建和验证；2026-08-10 完成 AreaSong Ops DNS、Access 与源站限制验收。Access 的当前事实以上表为准：

| 类别 | 当前状态 | 说明 |
| --- | --- | --- |
| DNS TTL | 全部已存在记录为自动 | `resume`、`sorryiossearch`、`monitor`、`forge`、`ops` 已代理；`cpa`、`log` 为 DNS-only；`www` 的旧 Access/Tunnel 入口已下线，当前仍由 Cloudflare 边缘响应并预留给门户网站。 |
