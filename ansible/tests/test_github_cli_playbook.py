...
PLAYBOOK_PATH = REPO_ROOT / "ansible" / "github-cli.yml"


class GitHubCliPlaybookTests(unittest.TestCase):
    def test_binary_source_is_versioned_and_checksum_pinned(self) -> None:
        ...
        variables = play["vars"]
        self.assertRegex(variables["github_cli_version"], r"^\d+\.\d+\.\d+$")
        self.assertRegex(variables["github_cli_sha256"], r"^[0-9a-f]{64}$")
        serialized = yaml.safe_dump(play)
        self.assertIn("https://github.com/cli/cli/releases/download/", serialized)
    ...
    def test_installed_binary_is_root_owned_and_not_world_writable(self) -> None:
        play = yaml.safe_load(PLAYBOOK_PATH.read_text(encoding="utf-8"))[0]
        install_task = next(
            task for task in play["tasks"] if task["name"] == "Install the verified GitHub CLI binary"
        )
        copy = install_task["ansible.builtin.copy"]
        self.assertEqual(copy["dest"], "/usr/local/bin/gh")
    ...
    def test_attestation_verifier_dependencies_are_installed(self) -> None:
        play = yaml.safe_load(PLAYBOOK_PATH.read_text(encoding="utf-8"))[0]
        package_task = next(
            task for task in play["tasks"] if task["name"] == "Install provenance verification dependencies"
        )
