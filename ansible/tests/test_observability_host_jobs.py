...
class ObservabilityHostJobsTests(unittest.TestCase):
    ...
    def test_all_host_jobs_are_managed(self) -> None:
        ...
        self.assertEqual(
            set(self.play["vars"]["cron_files"]),
            {
                ...
                "ops-alertmanager-runtime-input",
    ...
    def test_heavy_collectors_are_staggered_inside_flock(self) -> None:
        delays = {
            ...
            "ops-alertmanager-runtime-input": 20,
    ...
    def test_weekly_docker_cleanup_is_build_cache_only(self) -> None:
        ...
        self.assertIn('builder prune --all --force --filter "until=${RETENTION}"', script)
        self.assertIn("docker_build_cache_prune_last_success_timestamp", script)
        self.assertIn("docker_build_cache_prune_reclaimed_bytes", script)
        self.assertIn('reclaimed_bytes="${reclaimed_bytes%B}"', script)
