...
| 17 | 数据库与中间件 | 部分达标 | DB/Redis 不公网暴露；备份/exporter 已接入；Postgres 角色权限已只读复核；Redis 密码、maxmemory、AOF 已复核 | Redis 高危命令 / ACL 策略待决策；`sub2api` 运行用户仍为 superuser，需应用侧拆分 migration/runtime；无 PITR/WAL 归档 |
