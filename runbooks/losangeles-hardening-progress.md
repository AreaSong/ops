...
- `/opt/as_password` 是当前最高优先级风险：文件为明文密码且权限为 `0644`，本机所有用户可读。任务结束后应立即删除、清 sudo 缓存并修改密码。
...
1. 删除 `/opt/as_password` 并修改密码。  
   当前文件权限为 `-rw-r--r-- root root`，属于明文凭据暴露。建议任务结束后执行 `sudo -k`、删除文件，并由用户修改 `as` 密码。
...
1. 立即处理 `/opt/as_password`：任务结束后清 sudo 缓存、删除文件、修改密码。
2. 安排系统更新与重启维护窗口。
