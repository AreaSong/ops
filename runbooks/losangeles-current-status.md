...
1. 有固定出口 IP 后，限制 SSH 来源。
2. 有测试账号或应用配合后，补登录后业务指标和关键接口分位延迟。
3. 准备门户网站时，接入 `www.areasong.top`。
4. 有临时机器时，按跨机器恢复 Runbook 做一次实机演练。
5. 业务数据增长后，规划独立数据盘和数据迁移。
6. 有维护窗口时，再评估主机名规范化。
...
已新增 `runbooks/losangeles-standards-09-d2-cloud-control-plane-governance-20260705.md`，将 Cloudflare 已完成项、服务器侧已确认事实、云厂商控制台待确认项分开管理。

待用户在控制台确认：云账号 MFA、账单/到期/欠费告警、安全组/云防火墙、自动快照、云审计、安全通知、API Key 最小权限。
