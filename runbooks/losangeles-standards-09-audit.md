...
- 待优化：Redis 高危命令 / ACL 策略决策、sub2api migration/runtime 拆分实施、SSH 来源 IP 限制等待固定出口 IP。sub2api 直接切低权限失败原因已在 C2f 定位，C2g 已确认当前上游未发现独立 migration-only 命令或关闭启动自动 migration 的开关。
...
3. 批次 A、B1、B2、B3、C1、C3a、C3b、C3c 已完成；Postgres 角色权限只读复核、C2f sub2api migration/runtime 失败点定位和 C2g migration 能力分析已完成。后续进入需要维护窗口或业务配合的 Redis 高危命令 / ACL 策略、sub2api 应用侧 migration/runtime 拆分能力改造。
