...
`SyntheticSloBudgetExhausted` 表示滚动 30 天窗口中的历史失败已经消耗完
99.9% 可用性目标的错误预算，不等同于服务此刻仍不可用。当前探针恢复后，该告警仍会
保留到足够多的历史失败样本滚出 30 天窗口；不得通过降低目标、放宽探针超时或静默告警
伪造恢复。

处置时同时检查当前 `probe_success` 和 5 分钟、30 分钟、1 小时、6 小时燃烧率：若当前
探针为 1 且四个短窗口燃烧率均为 0，应记录为“当前故障已恢复、历史预算仍耗尽”，继续
保留预算告警并追溯历史失败；若当前探针或短窗口仍异常，则按活动故障继续处理。
