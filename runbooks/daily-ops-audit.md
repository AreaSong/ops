...
如日报造成明显 I/O 或错误告警，先移除 `/etc/cron.d/ops-daily-ops-audit` 停止后续调度；保留脚本、报告和日志作为证据。Alertmanager、Promtail 或 Prometheus 配置回滚到变更前备份并分别重建/reload，确认原有目标和告警均正常后结束回滚。
