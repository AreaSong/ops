...
上一份 `runbooks/losangeles-standards-09-audit.md` 主要是附录 A 的 P0 初步验收矩阵；本报告扩展到了 `standards/09` 31 章清单里的 P0/P1/P2 方向，覆盖系统、存储、软件源、账号、SSH、sudo、授权 key 指纹、UFW、Fail2ban、Nginx、TLS、Docker、Compose、数据库/Redis、备份恢复、监控告警、日志轮转、定时任务、凭据权限、台账与云侧待确认项。
...
以下无法从服务器内完整验证，必须在云厂商或 Cloudflare 控制台确认：

- 云主账号 MFA 是否开启。
- 云主账号是否不用于日常操作。
- 是否存在主账号 AccessKey。
- 程序类 AccessKey 是否最小权限。
- 安全组是否只放行 22/80/443，且无全端口放行。
- 账单、余额、到期、欠费告警是否开启。
- 实例是否为非抢占式/非竞价实例。
- 实例维护事件通知是否开启。
- 云审计日志是否开启且保留期足够。
- 云快照策略是否存在。
- 云标签 `env/role/project/owner` 是否完整。
- 域名自动续费和 DNS 托管账号 MFA。
...
仍未完成的是“云侧控制台确认”和“需要业务凭据/数据库密码配合的深层权限核验”，这些不能从主机内无凭据地可靠完成，已在本报告列为待确认项。
