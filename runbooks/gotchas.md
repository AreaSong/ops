...
- **受 `DisallowUnknownFields` 约束的配置扩展必须先做向后兼容**——新 Runner 若要求旧生产 `services.json` 立即补写字段，失败回滚到旧 Runner 时旧二进制会因未知字段进入重启循环。新增配置字段应先提供内存默认值并覆盖旧配置测试，确认新旧 Runner 都能读取后再安排生产迁移。
